...
object Converters {
    ...
    @TypeConverter
    fun timeFromTimestamp(value: Int?): kotlinx.datetime.LocalTime? {
        return value?.let { kotlinx.datetime.LocalTime.fromSecondOfDay(it) }
    }

    @TypeConverter
    fun timeToTimestamp(time: kotlinx.datetime.LocalTime?): Int? {
